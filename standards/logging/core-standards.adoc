...
* On validating you must ensure, that there is no "dangling" LogRecord, saying each type must be used. If it is not used, analyze the codebase, whether it has somewhere to be used. Remove it otherwise. Remove it from LogMessages.adoc as well
...
* LogMessages.adoc exists for each module
