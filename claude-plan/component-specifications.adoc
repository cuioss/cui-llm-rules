...
⚠️ **Environment Variable (Unverified)**: `${CLAUDE_PLUGIN_ROOT}` - This environment variable is referenced in the specification but **requires verification** that Claude Code actually provides it. If this variable does not exist, scripts requiring absolute paths will need alternative path resolution mechanisms.
----
