...
def test_persist_does_not_override_existing_commands():
    """Test persist does not override type-based commands with profile commands."""
    with ProfileTestContext() as temp_dir:
        # Create a project with quality-gate profile (which is also generated by type)
        (temp_dir / 'pom.xml').write_text('''<project>
            <profiles>
                <profile>
                    <id>quality</id>
        ...
        # quality-gate should use the type-based command (with -Ppre-commit), not profile-based
        assert 'quality-gate' in commands, "Should have quality-gate command"
        # Type-based uses -Ppre-commit, profile-based would use -Pquality
        assert '-Ppre-commit' in commands['quality-gate'], \
            f"Should use type-based command: {commands['quality-gate']}"
...
if __name__ == '__main__':
    ...
    runner.add_tests([
        ...
        test_persist_does_not_override_existing_commands,
