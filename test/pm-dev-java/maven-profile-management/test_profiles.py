...
def test_suggest_classifications():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        apache_suggestion = next(
            s for s in suggestions if s["profile_id"] == "apache-release"
        )
        assert apache_suggestion["suggested"] == "skip"

        # Find custom-profile suggestion
        custom_suggestion = next(
            s for s in suggestions if s["profile_id"] == "custom-profile"
        )
