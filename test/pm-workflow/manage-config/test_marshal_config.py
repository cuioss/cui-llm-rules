...
def test_domain_agents_set():
    ...
    with TestContext() as ctx:
        ...
        assert config['domain_agents']['pm-workflow:plan-type-java']['solution_outline_agent'] == 'test:solution-outline-agent'
...
def test_domain_agents_set_null():
    ...
    with TestContext() as ctx:
        ...
        config = json.loads(ctx.marshal_path.read_text())
        assert config['domain_agents']['pm-workflow:plan-type-java']['solution_outline_agent'] is None
...
def test_plan_type_defaults_set():
    ...
    with TestContext() as ctx:
        ...
        assert result.success
        config = json.loads(ctx.marshal_path.read_text())
        pt = config['plan_type_defaults']['pm-workflow:plan-type-java']
...
def test_rules_match_default():
    ...
    with TestContext():
        ...
        assert result.success
        data = parse_toon(result.stdout)
        assert data['data']['plan_type'] == 'pm-workflow:plan-type-generic'
...
def test_rules_match_specific():
    ...
    with TestContext() as ctx:
        ctx.write_marshal({
            "plan_type_rules": [
                {"pattern": "*.java", "plan_type": "pm-workflow:plan-type-java", "description": "Java"},
        ...
        assert result.success
        data = parse_toon(result.stdout)
        assert data['data']['plan_type'] == 'pm-workflow:plan-type-java'
...
def test_rules_match_or_pattern():
    ...
    with TestContext() as ctx:
        ctx.write_marshal({
            "plan_type_rules": [
                {"pattern": "*.js|*.ts|*.tsx", "plan_type": "pm-workflow:plan-type-javascript", "description": "JS"},
                {"pattern": "*", "plan_type": "pm-workflow:plan-type-generic", "description": "Default"}
            ]
        })
        ...
        assert result.success
        data = parse_toon(result.stdout)
        assert data['data']['plan_type'] == 'pm-workflow:plan-type-javascript'
...
def test_keywords_match():
    ...
    with TestContext() as ctx:
        ctx.write_marshal({
            "detection_keywords": {
                "pm-workflow:plan-type-java": ["java", "junit", "maven"]
            }
        })
