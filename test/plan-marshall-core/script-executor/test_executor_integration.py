...
class ExecutorTestEnvironment:
    ...
    def _discover_test_scripts(self) -> dict:
        ...
        test_scripts = [
            ('planning', 'manage-config', 'manage-config.py'),
            ('planning', 'manage-lifecycle', 'manage-lifecycle.py'),
            ('general-tools', 'toon-usage', 'toon_parser.py'),
        ]
...
def test_executor_contains_mappings():
    ...
    assert 'planning:manage-config' in content, "Missing planning:manage-config mapping"
...
def test_executor_list_command():
    ...
    assert 'planning:manage-config' in output, f"Missing planning:manage-config in list: {output}"
...
def test_execute_script_help():
    ...
    result = env.run_executor('planning:manage-config', '--help')
...
def test_execute_script_with_subcommand():
    ...
    result = env.run_executor('planning:manage-lifecycle', '--help')
...
def test_log_format_success_compact():
    ...
    env.run_executor('planning:manage-config', '--help')
...
def test_execute_script_that_fails():
    ...
    result = env.run_executor('planning:manage-config', 'get')
...
def test_error_execution_logged_with_details():
    ...
    result = env.run_executor('planning:manage-config', 'get', '--plan-id', 'nonexistent-plan-xyz')

    # This should fail (plan doesn't exist)
    assert result.returncode != 0, "Expected non-zero exit for nonexistent plan"

    log_content = env.get_log_content()
    assert 'planning:manage-config' in log_content, f"Missing log entry: {log_content}"
...
def test_log_format_error_multi_line():
    ...
    result = env.run_executor('planning:manage-config', 'get', '--plan-id', 'test-error-format')
    ...
    error_lines = [l for l in lines if 'planning:manage-config' in l or l.startswith('  ')]
...
def test_subcommand_forwarded():
    ...
    result = env.run_executor('planning:manage-config', 'get', '--help')
...
def test_partial_notation_resolution():
    ...
    # The executor has fuzzy matching - 'planning' should match 'planning:*'
    result = env.run_executor('planning', '--help')
...
def test_partial_notation_matches_substring():
    ...
    # 'manage-config' should match 'planning:manage-config'
...
def test_multiple_executions_append_to_log():
    ...
    # Count occurrences
    count = log_content.count('planning:manage-config')
...
def test_plan_scoped_log_when_plan_exists():
    ...
    try:
        ...
        assert 'planning:manage-config' in log_content, \
