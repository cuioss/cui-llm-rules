...
# Script under test
SCRIPT_PATH = get_script_path('planning', 'planning-inventory', 'scan-planning-inventory.py')
...
def test_core_has_required_fields():
    ...
    assert core['bundle'] == 'planning', "Core bundle should be 'planning'"
...
def test_derived_includes_java_expert():
    """Test derived includes cui-java-expert bundle."""
    result = run_script(SCRIPT_PATH)
    assert result.returncode == 0, f"Script returned error: {result.stderr}"
    ...
    assert 'cui-java-expert' in bundle_names, "Derived should include cui-java-expert"


def test_derived_java_has_plan_components():
    """Test cui-java-expert derived bundle has plan components."""
    result = run_script(SCRIPT_PATH)
    assert result.returncode == 0, f"Script returned error: {result.stderr}"
...
def test_derived_includes_frontend_expert():
    """Test derived includes cui-frontend-expert bundle."""
    result = run_script(SCRIPT_PATH)
    assert result.returncode == 0, f"Script returned error: {result.stderr}"
    ...
    derived = data.get('derived', [])
    bundle_names = [d['bundle'] for d in derived]
    assert 'cui-frontend-expert' in bundle_names, "Derived should include cui-frontend-expert"


def test_derived_includes_plugin_tools():
    """Test derived includes cui-plugin-development-tools bundle."""
    result = run_script(SCRIPT_PATH)
    assert result.returncode == 0, f"Script returned error: {result.stderr}"
    ...
    derived = data.get('derived', [])
    bundle_names = [d['bundle'] for d in derived]
    assert 'cui-plugin-development-tools' in bundle_names, "Derived should include cui-plugin-development-tools"
