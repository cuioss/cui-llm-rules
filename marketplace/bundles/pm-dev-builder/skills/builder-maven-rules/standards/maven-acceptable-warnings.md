...
Use the `plan-marshall:run-config` skill for all configuration access.
...
1. **Load patterns**: Activate `plan-marshall:run-config` to read `maven.acceptable_warnings`
