...
```bash
echo '{llm_findings_json}' | python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate cross-file --analysis {script_analysis_json}
```
...
1. **Run cross-file analysis script**:
   ```bash
   python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze cross-file --skill-path {skill_path} > analysis.json
   ```
...
5. **Verify LLM findings**:
   ```bash
   echo '{llm_output}' | python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate cross-file --analysis analysis.json
   ```
