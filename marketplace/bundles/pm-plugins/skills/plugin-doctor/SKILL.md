...
1. **MANDATORY - Load Prerequisites**
   ...
   **EXECUTE** these skill loads before proceeding:
   ...
   Skill: pm-plugins:plugin-architecture
...
4. **MANDATORY - Analyze Each Component** (using scripts)
   ...
   Scripts:
   - `pm-plugins:plugin-doctor` → `analyze.py markdown`
   - `pm-plugins:plugin-doctor` → `analyze.py coverage`
   - `pm-plugins:plugin-doctor` → `validate.py references`

   **EXECUTE**:
   ```bash
   python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze markdown --file {path} --type {type}
   python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze coverage --file {path}
   python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate references --file {path}
   ```
...
1. **Verify Fixes**
   ...
   Script: `pm-plugins:plugin-doctor` → `fix.py verify`

   ```bash
   python3 .plan/execute-script.py pm-plugins:plugin-doctor:fix verify --fix-type {type} --file {path}
   ```
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze markdown --file {agent_path} --type agent
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze coverage --file {agent_path}
python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate references --file {agent_path}
```
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze markdown --file {cmd_path} --type command
python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate references --file {cmd_path}
```
...
**Check criteria**:
1. Extract skill invocations from command (e.g., `Skill: pm-plugins:plugin-create`)
...
Scripts:
- `pm-plugins:plugin-doctor` → `analyze.py structure`
- `pm-plugins:plugin-doctor` → `analyze.py markdown`
- `pm-plugins:plugin-doctor` → `validate.py references`

```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze structure --directory {skill_dir}
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze markdown --file {skill_dir}/SKILL.md --type skill
python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate references --file {skill_dir}/SKILL.md
```
...
**Check criteria**:
1. Search SKILL.md for `Skill: pm-plugins:plugin-architecture`
2. Search SKILL.md for `Skill: pm-core:diagnostic-patterns`
...
Skill: pm-core:diagnostic-patterns
Skill: pm-plugins:plugin-architecture
Skill: pm-plugins:plugin-script-architecture
Read references/fix-catalog.md
```
...
Script: `pm-plugins:plugin-doctor` → `validate.py inventory`

```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate inventory --skill-path {skill_path}
```
...
Script: `pm-plugins:plugin-doctor` → `analyze.py cross-file`

```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze cross-file --skill-path {skill_path}
```
...
Script: `pm-plugins:plugin-doctor` → `validate.py cross-file`

Pipe LLM findings JSON to verification:
```bash
echo '{llm_findings_json}' | python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate cross-file --analysis {cross_file_analysis_json}
```
...
Script: `pm-plugins:plugin-doctor` → `validate.py references`

```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate references --file {skill_path}/SKILL.md
```
...
- `Skill(pm-core:*)` - diagnostic-patterns
- `Skill(pm-plugins:*)` - plugin-architecture, marketplace-inventory
