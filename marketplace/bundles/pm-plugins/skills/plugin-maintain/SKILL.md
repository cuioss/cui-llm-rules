...
Script: `pm-plugins:plugin-maintain` → `maintain.py`
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-maintain:maintain analyze --component {component_path}
```
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-maintain:maintain update --component {component_path} --updates '{"updates": [...]}'
```
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-maintain:maintain check-duplication --skill-path {skill_path} --content-file {content_file}
```
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-maintain:maintain readme --bundle-path {bundle_path}
```
