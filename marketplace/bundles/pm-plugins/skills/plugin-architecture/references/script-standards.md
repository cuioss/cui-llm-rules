...
**Example**:
```
marketplace/bundles/pm-plugins/skills/plugin-diagnose/
└── scripts/
...
```bash
python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze markdown --file {file_path} --type agent
```
...
- **Usage**:
  ```bash
  python3 .plan/execute-script.py pm-plugins:plugin-doctor:analyze coverage --file {file_path}
  ```
...
- **Usage**:
  ```bash
  python3 .plan/execute-script.py pm-plugins:plugin-doctor:validate references --file {file_path}
  ```
```
...
**Pattern**: `test/pm-plugins/{skill-name}/test-{script-name}.sh`

**Example**:
```
test/pm-plugins/plugin-diagnose/
...
**Example**:
```
test/pm-plugins/plugin-diagnose/fixtures/
...
**Manual Execution**:
```bash
cd test/pm-plugins/plugin-diagnose/
./test-analyze-markdown-file.sh
```
...
python3 .plan/execute-script.py pm-plugins:plugin-doctor:fix extract --input file.md | \
  python3 .plan/execute-script.py pm-plugins:plugin-doctor:fix categorize | \
  python3 .plan/execute-script.py pm-plugins:plugin-doctor:fix apply | \
  python3 .plan/execute-script.py pm-plugins:plugin-doctor:fix verify
```
