...
2. **Run Coverage Analysis Script**
   ...
   Script: `pm-java:cui-java-unit-testing` → `coverage.py`

   ```bash
   python3 .plan/execute-script.py pm-java:cui-java-unit-testing:coverage analyze --file {report_path} --threshold {threshold}
   ```
...
2. **Analyze Coverage Gaps**
   ...
   ```bash
   python3 .plan/execute-script.py pm-java:cui-java-unit-testing:coverage gaps --report {report_path} --priority {priority_filter}
   ```
...
- **Location**: `pm-java:cui-java-unit-testing/analyze-coverage-gaps`
