...
Record improvements: `Skill: cui-utilities:claude-lessons-learned` with component `{type: "command", name: "pr-doctor", bundle: "cui-task-workflow"}`
...
After fixes: Verify via /maven-build-and-fix, commit via cui-git-workflow, push to PR branch.
...
/pr-doctor (orchestrator)
  ...
  ├─> cui-git-workflow skill (Commit workflow)
