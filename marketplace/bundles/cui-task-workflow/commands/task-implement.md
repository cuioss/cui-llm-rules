...
Record improvements: `Skill: cui-utilities:claude-lessons-learned` with component `{type: "command", name: "task-implement", bundle: "cui-task-workflow"}`
...
Run `SlashCommand(/builder-maven:maven-build-and-fix)`. Iterate up to 3 times if fails.

### Step 5: Commit and Push

If verification succeeds: Commit via `cui-git-workflow` skill (Commit workflow).
...
- **claude-memory** skill - State persistence for recovery
