...
| System | Skill |
|--------|-------|
| maven | `plan-marshall:build-operations` |
| gradle | `plan-marshall:build-operations` |
| npm | `plan-marshall:build-operations` |
...
python3 .plan/execute-script.py plan-marshall:plan-marshall-config:plan-marshall-config \
  ...
  --command 'python3 .plan/execute-script.py plan-marshall:build-operations:maven execute --goals "clean verify -DskipITs"'
```
...
The `plan-marshall:build-operations` skill provides:

1. **Detection**: `build_env detect` - Detect available systems
2. **Module Detection**: `build_env detect-modules` - Detect project modules
3. **Command Generation**: `build_env persist` - Generate and save commands
4. **Execution**: `maven/gradle/npm execute` - Execute builds
...
3. Command invokes build script:
   python3 .plan/execute-script.py plan-marshall:build-operations:maven execute --goals "..."
```
