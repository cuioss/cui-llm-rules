...
2. **Run coverage analysis script**
   ...
   Script: `cui-frontend-expert:cui-javascript-unit-testing` → `js-coverage.py`

   ```bash
   python3 {script_path} analyze --report coverage/coverage-summary.json
   # Or for LCOV format:
   python3 {script_path} analyze --report coverage/lcov.info --format lcov
   # With custom threshold:
   python3 {script_path} analyze --report coverage/coverage-summary.json --threshold 80
   ```
