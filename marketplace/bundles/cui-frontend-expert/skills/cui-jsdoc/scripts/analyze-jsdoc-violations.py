#!/usr/bin/env python3
"""
Analyze JavaScript files for JSDoc compliance violations.

Returns structured JSON with violations categorized by severity (CRITICAL, WARNING, SUGGESTION).
Stdlib-only - no external dependencies required.

Usage:
    python3 analyze-jsdoc-violations.py --directory src/
    python3 analyze-jsdoc-violations.py --file src/utils/formatter.js
    python3 analyze-jsdoc-violations.py --directory src/ --scope missing

Output:
    JSON object with status, violations list, and summary metrics
"""
...
def find_preceding_jsdoc(content: str, pos: int) -> str | None:
    ...
    # Look backwards from position to find potential JSDoc
    preceding = content[:pos].rstrip()

    # Find last JSDoc end marker
    jsdoc_end = preceding.rfind('*/')
    if jsdoc_end == -1:
        return None

    # Check if JSDoc is immediately before (only whitespace between)
    between = preceding[jsdoc_end + 2:]
    if between.strip():
        return None

    # Find start of this JSDoc
...
def extract_function_params(param_str: str) -> list[str]:
    ...
    for char in param_str:
        if char in '({[':
            ...
            param = current.strip()
            if param:
                # Extract name (before = for defaults, handle destructuring)
                if '=' in param:
                    param = param.split('=')[0].strip()
                if param.startswith('{') or param.startswith('['):
                    # Destructured param - use the variable name
                    pass
                else:
                    params.append(param)
            current = ''
...
def check_function_jsdoc(jsdoc: str, params: list[str], has_return: bool) -> list[dict]:
    ...
    # Check @returns (if function likely returns a value)
...
def analyze_file(file_path: str, scope: str) -> list[dict]:
    ...
    # Check for @fileoverview (suggestion only)
    ...
    for match in FUNCTION_PATTERN.finditer(content):
        ...
        if scope in ('all', 'syntax') and jsdoc:
            ...
            # Simple heuristic for return detection
            ...
            if func_end != -1:
                ...
                while pos < len(content) and brace_count > 0:
                    if content[pos] == '{':
                        ...
                        # Check it's actually a return statement
...
def main():
    parser = argparse.ArgumentParser(
        description='Analyze JavaScript files for JSDoc compliance violations',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog='''
Examples:
  %(prog)s --directory src/
  %(prog)s --file src/utils/formatter.js
  %(prog)s --directory src/ --scope missing
...
Scope options:
  ...
  syntax  - Only check JSDoc syntax and completeness
    ...
    group.add_argument(
        '--file',
        help='Single JavaScript file to analyze'
    )
