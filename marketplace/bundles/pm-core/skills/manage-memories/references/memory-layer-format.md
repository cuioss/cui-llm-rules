...
```bash
python3 .plan/execute-script.py pm-core:manage-memories:manage-memory save \
  --category context \
...
```bash
python3 .plan/execute-script.py pm-core:manage-memories:manage-memory load \
  --category context \
  --identifier "task-42"
```
...
```bash
python3 .plan/execute-script.py pm-core:manage-memories:manage-memory list \
  --category context \
  --since 7d
```
...
```bash
python3 .plan/execute-script.py pm-core:manage-memories:manage-memory query \
  --pattern "auth*" \
  --category context
```
...
```bash
python3 .plan/execute-script.py pm-core:manage-memories:manage-memory cleanup \
  --category context \
  --older-than 7d
```
