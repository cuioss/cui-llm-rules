...
**COMPLEMENTARY SKILL**: Use `pm-core:permission-doctor` for read-only analysis before applying fixes.

## Script Reference

| Script | Notation | Purpose |
|--------|----------|---------|
| permission-fix | `pm-core:permission-fix:permission-fix` | Write operations for permissions |
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix apply-fixes \
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix add \
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix remove \
  --permission "Bash(docker:*)" \
  --target project
```
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix ensure \
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix consolidate \
  --settings ~/.claude/settings.json \
  --dry-run
```
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix ensure-wildcards \
  --settings ~/.claude/settings.json \
