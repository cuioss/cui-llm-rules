...
```python
sys.path.insert(0, str(Path(__file__).resolve().parents[4] / 'pm-core' / 'skills' / 'file-operations-base' / 'scripts'))
from file_ops import atomic_write_file, base_path, output_success, output_error
```
...
When scripts in one domain (e.g., `pm-workflow:plan-files`) need to access resources in another domain (e.g., `pm-core:manage-lessons-learned`), follow the **ID-based access pattern**.
