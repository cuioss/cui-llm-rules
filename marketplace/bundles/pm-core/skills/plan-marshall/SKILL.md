...
|--------|----------|---------|
| determine-mode | `pm-core:plan-marshall:determine-mode` | Determine wizard vs menu mode |
...
**Generate executor**:
1. Read template from: `marketplace/bundles/pm-core/skills/script-executor/templates/execute-script.py.template`
...
If yes:
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix apply-fixes --scope project
```
...
```bash
python3 .plan/execute-script.py pm-core:permission-fix:permission-fix apply-fixes --scope global
```
