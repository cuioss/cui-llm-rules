#!/usr/bin/env python3
"""
npm/npx build operations - run, execute, and parse.

Usage:
    npm.py run --targets <targets> [options]
...
def load_acceptable_warnings(project_dir: str = '.') -> dict:
    """Load acceptable_warnings from run-configuration.json for npm.
    ...
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        return config.get('npm', {}).get('acceptable_warnings', {})
...
def format_toon_error(error_type: str, message: str, log_file: str, exit_code: int, duration_ms: int, command: str, issues: list = None, mode: str = "actionable") -> str:
        ...
        if mode != "errors" and warnings:
            ...
            if mode == "structured":
                ...
                for w in warnings[:10]:
                    ...
                    lines.append(f"{file}    {line_num}    {msg}    {accepted}")
            else:
...
def cmd_run(args):
    ...
    # Handle timeout
    if exit_code == 124:
        print(format_toon_error("timeout", f"Build timed out after {args.timeout}ms", log_file, exit_code, duration_ms, display_cmd))
        return 1

    # Success case
    if exit_code == 0:
        print(format_toon_success(log_file, 0, duration_ms, display_cmd))
        return 0
