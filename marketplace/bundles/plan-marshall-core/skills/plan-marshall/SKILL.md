...
|--------|----------|---------|
| determine-mode | `plan-marshall-core:plan-marshall:determine-mode` | Determine wizard vs menu mode |
| gitignore-setup | `plan-marshall-core:plan-marshall:gitignore-setup` | Configure .gitignore for .plan/ |
| marshal-config | `planning:manage-config:marshal-config` | Project-level marshal.json CRUD |
| scan-marketplace-inventory | `plan-marshall-core:marketplace-inventory:scan-marketplace-inventory` | Script discovery |
| build-env | `builder:environment-detection:build-env` | Build system detection |
| permission-doctor | `plan-marshall-core:permission-doctor:permission-doctor` | Permission analysis |
| permission-fix | `plan-marshall-core:permission-fix:permission-fix` | Permission fixes |
| marketplace-sync | `plan-marshall-core:marketplace-sync:marketplace-sync` | Marketplace permission sync |
...
```bash
python3 .plan/execute-script.py plan-marshall-core:marketplace-sync:marketplace-sync ensure-executor \
  --target global
```
...
If yes:
```bash
python3 .plan/execute-script.py plan-marshall-core:permission-fix:permission-fix apply-fixes --scope project
```
...
```bash
python3 .plan/execute-script.py plan-marshall-core:permission-fix:permission-fix apply-fixes --scope global
```
