...
```python
sys.path.insert(0, str(Path(__file__).resolve().parents[4] / 'general-tools' / 'skills' / 'file-operations-base' / 'scripts'))
from file_ops import atomic_write_file, base_path, output_success, output_error
```
...
When scripts in one domain (e.g., `planning:plan-files`) need to access resources in another domain (e.g., `plan-marshall-core:manage-lessons-learned`), follow the **ID-based access pattern**.
