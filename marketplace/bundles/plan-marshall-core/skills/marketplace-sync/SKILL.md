...
|--------|----------|---------|
| marketplace-sync | `plan-marshall-core:marketplace-sync:marketplace-sync` | Marketplace permission synchronization |
...
python3 .plan/execute-script.py plan-marshall-core:marketplace-inventory:scan-marketplace-inventory \
  --scope marketplace --resource-types skills,commands | \
python3 .plan/execute-script.py plan-marshall-core:marketplace-sync:marketplace-sync generate-wildcards
...
```json
{
  ...
  "permissions": {
    ...
    "command_bundle_wildcards": ["SlashCommand(/builder:*)", "SlashCommand(/planning:*)"],
...
```bash
python3 .plan/execute-script.py plan-marshall-core:marketplace-sync:marketplace-sync cleanup-scripts \
  --target global \
  --remove-broad-python \
  --dry-run
```
...
```bash
python3 .plan/execute-script.py plan-marshall-core:marketplace-sync:marketplace-sync migrate-executor \
  --target global \
  --remove-broad-python \
  --dry-run
```
