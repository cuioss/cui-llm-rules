...
Usage:
    python3 generate-readme.py <bundle_path>

Arguments:
    bundle_path    Path to the bundle directory (must contain plugin.json)
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate README.md content from bundle inventory"
    )
    parser.add_argument(
        'bundle_path',
    ...
    if not bundle_path.exists():
        print(json.dumps({'error': f'Bundle directory not found: {args.bundle_path}'}))
        return 1

    if not bundle_path.is_dir():
        print(json.dumps({'error': f'Not a directory: {args.bundle_path}'}))
        return 1
    ...
    if not plugin_json.exists():
        plugin_json = bundle_path / 'plugin.json'
    if not plugin_json.exists():
        print(json.dumps({'error': f'Missing plugin.json in bundle: {args.bundle_path}'}))
        return 1
