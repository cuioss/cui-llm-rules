...
Reads fix details and applies the appropriate transformation
based on the fix type. Creates backup before modifying.

Usage:
    apply-fix.py <fix_json_file> <bundle_dir>
    echo '{"type": "...", "file": "..."}' | apply-fix.py - /path/to/bundle

Output: JSON with success/failure and details
"""
...
def apply_rule_6_fix(file_path: Path, fix: dict, templates: dict) -> dict:
    ...
    # Pattern to remove Task from tools line
    # Handles: Task, | , Task | Task (alone)
    patterns = [
        (r'^(tools:.*),\s*Task\b', r'\1'),           # Remove ", Task"
        (r'^(tools:.*)\bTask,\s*', r'\1'),           # Remove "Task, "
        (r'^(tools:\s*)Task$', r'\1Read'),           # Replace lone "Task" with "Read"
    ]
...
def apply_unused_tool_fix(file_path: Path, fix: dict, templates: dict) -> dict:
    ...
    for tool in unused_tools:
        ...
        patterns = [
            (rf',\s*{tool}\b', ''),        # Remove ", Tool"
            (rf'\b{tool},\s*', ''),        # Remove "Tool, "
        ]
        for pattern, replacement in patterns:
            new_content, count = re.subn(pattern, replacement, new_content, flags=re.MULTILINE)
            if count > 0:
...
def apply_pattern_22_fix(file_path: Path, fix: dict, templates: dict) -> dict:
    ...
    # Replace self-update commands with caller reporting
    replacements = [
...
# Map fix types to fix functions
FIX_HANDLERS = {
...
def apply_fix(fix: dict, bundle_dir: Path, templates: dict) -> dict:
    """
    Apply a single fix to a component file.

    Args:
        fix: Fix details with type, file, etc.
        bundle_dir: Path to bundle directory
        templates: Fix templates from assets
...
def main():
    parser = argparse.ArgumentParser(
        description="Apply a single fix to a component file"
    )
    parser.add_argument(
        "fix_file",
    ...
    try:
        ...
        # Apply fix
        result = apply_fix(fix, bundle_path, templates)
        print(json.dumps(result, indent=2))
