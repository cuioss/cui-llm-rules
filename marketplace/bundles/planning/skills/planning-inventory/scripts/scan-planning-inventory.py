...
PLANNING_PATTERNS = [
    ...
    "*-task-plan",       # Derived plan skills (java-plan, js-plan, etc.)
    "*-goals",      # Derived goals skills (java-solution-outline, js-solution-outline, etc.)
    "*-plan-*",     # Derived agents (java-task-plan-agent, etc.)
...
def find_marketplace_inventory_script() -> Path:
    ...
    # Try from cwd
    cwd_path = Path.cwd() / "marketplace/bundles/cui-plugin-development-tools/skills/marketplace-inventory/scripts/scan-marketplace-inventory.py"
    if cwd_path.exists():
        return cwd_path
