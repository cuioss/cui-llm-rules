...
3. **Analyze Issue Content**
   ...
   Resolve: planning:cui-task-planning/scripts/create-task-breakdown.py
   ```
   ```bash
...
1. **Parse Plan File**
   ```
   Skill: general-tools:script-runner
   Resolve: planning:cui-task-planning/scripts/track-task-progress.py
   ```
   ```bash
...
5. **Verify Acceptance Criteria**
   ```
   Skill: general-tools:script-runner
   Resolve: planning:cui-task-planning/scripts/validate-acceptance.py
   ```
   ```bash
...
### Related Skills
- **git-workflow** - Commit message format for task completion
- **cui-java-core** / **cui-javascript** - Standards during execution
- **general-tools:manage-memories** - Plan persistence across sessions
- **planning:workflow-patterns** - Handoff templates for Execute workflow
