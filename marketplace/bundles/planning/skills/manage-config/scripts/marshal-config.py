    ...
    if args.verb == 'match':
        ...
        for rule in rules:
            if match_pattern(rule['pattern'], filepath):
                ...
                return EXIT_SUCCESS
        output(success_response({"plan_type": "planning:plan-type-generic", "pattern": "*", "description": "Default"}))
        return EXIT_SUCCESS
...
def cmd_init(args) -> int:
    ...
    if hasattr(args, 'template') and args.template:
        template_path = Path(__file__).parent.parent.parent.parent.parent / 'planning' / 'skills' / 'plan-marshall' / 'templates' / f'{args.template}.json'
