...
```bash
python3 scripts/specification.py add \
  --plan-id {plan_id} \
...
```bash
python3 scripts/specification.py update \
  --plan-id {plan_id} \
  --number 2 \
  [--title "New title"] \
...
```bash
python3 scripts/specification.py remove \
  --plan-id {plan_id} \
  --number 2
```
...
```bash
python3 scripts/specification.py list \
  --plan-id {plan_id} \
...
```bash
python3 scripts/specification.py findAll \
  --plan-id {plan_id}
```
...
```bash
python3 scripts/specification.py get \
  --plan-id {plan_id} \
  --number 2
```
...
```bash
python3 scripts/specification.py check \
  --plan-id {plan_id} \
  --number 2 \
  --status done
```
...
```bash
python3 scripts/specification.py findByRequirement \
  --plan-id {plan_id} \
  --requirement REQ-1
```
...
|--------|---------|-------|
| `specification.py` | All CRUD operations via subcommands | `python3 scripts/specification.py {command} --help` |
...
```
Skill: planning:plan-specifications
operation: add
plan_id: {plan_id}
title: "JWT Token Format"
requirements: "REQ-1,REQ-3"
