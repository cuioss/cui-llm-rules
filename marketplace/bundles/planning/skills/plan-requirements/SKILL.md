...
```bash
python3 scripts/requirement.py add \
  --plan-id {plan_id} \
...
```bash
python3 scripts/requirement.py update \
  --plan-id {plan_id} \
  --number 2 \
  [--title "New title"] \
  [--body "New body"] \
...
```bash
python3 scripts/requirement.py remove \
  --plan-id {plan_id} \
  --number 2
```
...
```bash
python3 scripts/requirement.py list \
  --plan-id {plan_id} \
...
```bash
python3 scripts/requirement.py findAll \
  --plan-id {plan_id}
```
...
```bash
python3 scripts/requirement.py get \
  --plan-id {plan_id} \
  --number 2
```
...
```bash
python3 scripts/requirement.py check \
  --plan-id {plan_id} \
  --number 2 \
  --status done
```
...
|--------|---------|-------|
| `requirement.py` | All CRUD operations via subcommands | `python3 scripts/requirement.py {command} --help` |
...
```
Skill: planning:plan-requirements
operation: add
plan_id: {plan_id}
title: "Implement user authentication"
