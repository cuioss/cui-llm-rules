...
2. **Fetch Issues**
   ...
   ```bash
   python3 .plan/execute-script.py planning:sonar-workflow:sonar fetch --project {key} [--pr {id}]
   ```
...
2. **Triage Each Issue**
   ...
   ```bash
   python3 .plan/execute-script.py planning:sonar-workflow:sonar triage --issue '{json}'
   ```
...
**Usage:**
```bash
python3 .plan/execute-script.py planning:sonar-workflow:sonar fetch --project <key> [--pr <id>] [--severities <list>]
```
...
**Usage:**
```bash
python3 .plan/execute-script.py planning:sonar-workflow:sonar triage --issue '{"key":"...", "rule":"...", ...}'
```
