...
- When user provides a task description, you MUST create plans via `planning:plan-files` skill first
...
|--------|-----------------------------------|------------------------------|
| Storage | `~/.claude/plans/` or `.claude/plans/` | `planning:plan-files` skill |
...
2. Delegate to `Skill: planning:plan-init` with task_description/issue_url
...
5. Delegate to `Skill: planning:plan-refine`
...
2. **Route based on phase**:
   ...
   **If phase is `init` or `refine`:**
   - Delegate directly to `Skill: planning:plan-refine`
   ...
   **If phase is `execute`, `verify`, or `finalize`:**
   ...
   - If "Go back to refine":
     ...
     - Then delegate to `Skill: planning:plan-refine`
...
3. **If creating new**:
   - Delegate to `Skill: planning:plan-init`
...
3. **Delegate to `Skill: planning:plan-refine`**
...
1. **Query lessons via skill script**:
   ...
   ```
   Script path from: `general-tools:manage-lessons-learned/scripts/query-lessons.py`
...
All scripts use portable notation: `planning:phase-management/scripts/{script-name}`

| Script | Notation | Purpose |
|--------|----------|---------|
| `discover-plans.py` | `planning:phase-management/scripts/discover-plans.py` | Find plans in workspace |
...
```bash
python3 {write-lesson.py path} --component "planning:phase-management" --category {bug|improvement|anti-pattern} --title "Brief description" --detail "What happened, why, workaround, suggested fix"
```
