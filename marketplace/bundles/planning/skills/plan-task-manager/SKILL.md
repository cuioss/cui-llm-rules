...
```bash
python3 scripts/task.py remove \
  --plan-id {plan_id} \
  --number 1
```
...
```bash
python3 scripts/task.py step-start \
  --plan-id {plan_id} \
  --task 2 \
  --step 2
```
...
```bash
python3 scripts/task.py step-done \
  --plan-id {plan_id} \
  --task 2 \
  --step 2
```
...
```bash
python3 scripts/task.py step-skip \
  --plan-id {plan_id} \
  --task 2 \
  --step 2 \
...
```bash
python3 scripts/task.py add-step \
  --plan-id {plan_id} \
  --task 2 \
...
```bash
python3 scripts/task.py remove-step \
  --plan-id {plan_id} \
  --task 2 \
  --step 3
```
...
|--------|---------|-------|
| `task.py` | All CRUD and step operations via subcommands | `python3 scripts/task.py {command} --help` |
...
FOR EACH specification:
  ...
  3. CREATE tasks for the specification:
     task.py add --plan-id {plan_id} --specification SPEC-{n} ...
```
...
```
LOOP:
  1. QUERY next: task.py next --plan-id {plan_id}
  2. IF no next: DONE
  3. MARK started: task.py step-start ...
  4. EXECUTE step
  5. ON SUCCESS: task.py step-done ...
  6. CONTINUE
```
...
Query capabilities:
- `requirement.py findAll` -> All requirements
- `specification.py findByRequirement REQ-1` -> SPEC-1
- `task.py list --specification SPEC-1` -> TASK-1, TASK-2
- `task.py next` -> Next actionable step
