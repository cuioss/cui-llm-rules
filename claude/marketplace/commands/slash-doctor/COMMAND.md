...
- **command-name** (optional): Review a specific command by name (e.g., `verify-project`)
...
- Process all `.md` files in `~/.claude/commands/` directory
- Exclude `slash-doctor.md` (this file) from analysis
...
# For global scope
find ~/.claude/commands -name "*.md" -type f ! -name "slash-doctor.md" | sort
...
**Note**: Do NOT check if command includes permission verification, self-analysis, or other meta-features. Those are slash-doctor's responsibilities, not the command's.
...
**SIMPLIFIED WORKFLOW**: slash-doctor just extracts, setup-project-permissions handles all the verification and fixing.
...
- <command-1>: <issue>
- <command-2>: <issue>
Re-run slash-doctor on these commands to fix.
</if>
...
The slash-doctor command must IMPROVE commands without making them longer or more complex. Follow these strict rules:
